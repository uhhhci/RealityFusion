...
// Perform initial steps for each Gaussian prior to rasterization.
template<int C>
__global__ void preprocessCUDAUnity(int P, int D, int M,
	const float* orig_points,
	const glm::vec3* scales,
	const float scale_modifier,
	const glm::vec4* rotations,
	...
	const int* selected)
{
	...
	// masking the remove && reveal effects
	if(mask[idx] == 0)
		return;
	...
	float* __restrict__ out_depth)
{
	...
	if (inside)
	{
		...
		for (int ch = 0; ch < CHANNELS; ch++)
			...
			//out_color[pix_id*CHANNELS +ch] = C[ch] + T * bg_color[ch]; // use this when copying to CPU
		out_depth[pix_id] = D;
	}
}
...
void FORWARD::preprocessUnity(int P, int D, int M,
	const float* means3D,
	const glm::vec3* scales,
	const float scale_modifier,
	const glm::vec4* rotations,
	...
	const int* selected)
{
	preprocessCUDAUnity<NUM_CHANNELS> << <(P + 255) / 256, 256 >> > (
		P, D, M,
		means3D,
		scales,
		scale_modifier,
