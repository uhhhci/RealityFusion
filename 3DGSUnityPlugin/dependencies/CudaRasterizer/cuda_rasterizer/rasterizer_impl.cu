...
__global__ void maskSphere( int P, 
							const float* orig_points,
							int* mask,
							const float* center,
							const float* radius)
{
	...
	float3 c = { center[0], center[1], center[2] };
	// Calculate the distance between the point and the center
	float distance = sqrt((p_orig.x - c.x) * (p_orig.x - c.x) + 
						  (p_orig.y - c.y) * (p_orig.y - c.y) + 
						  (p_orig.z - c.z) * (p_orig.z - c.z));
...
void CudaRasterizer::Rasterizer::hideSphereContent(int P, 
					   const float* orig_points,
					   int* mask,
					   const float* center,
					   const float* radius)
{
	 maskSphere << <(P + 255) / 256, 256 >> > (
		P,
		orig_points,
		mask, 
		center,
...
// Forward rendering procedure for differentiable rasterization
// of Gaussians.
int CudaRasterizer::Rasterizer::forwardUnity(
	std::function<char* (size_t)> geometryBuffer,
	std::function<char* (size_t)> binningBuffer,
	std::function<char* (size_t)> imageBuffer,
	const int P, int D, int M,
