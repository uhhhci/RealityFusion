...
/// </summary>
public class ZEDCustomPointCloudManager : MonoBehaviour
{
    /// <summary>
    ...
    /// </summary>
    /// 
    private const int SET_ZED_CLOUD = 0x0007;
    ...
    // test transformation matrix to see if the results are correct
    private Matrix4x4 m_transform_zed_gs;
    void Start()
    {
        if (zedManager == null)
        {
        ...
        if (zed.IsCameraReady) //Don't do anything unless the ZED has been initialized. 
        {
            if (numberPoints == 0)
            {
                ...
                if (mat != null)
                {
                    ...
                    mat.SetMatrix("_GSTransformOffset", m_transform_zed_gs);


                }
                // update native plugin!
                GaussianRFRenderPlugin.update_ZED_status(zed.IsCameraReady, XYZTexture.width, XYZTexture.height, XYZTexture.GetNativeTexturePtr());
    ...
    public void SetZedPointCloudInNativePlugin()
    {
        if(zed.IsCameraReady && numberPoints != 0)
        {
            GL.IssuePluginEvent(GaussianRFRenderPlugin.GetRenderEventFunc(), SET_ZED_CLOUD);
        }
    }

    public void RegisterzedPointCloudwithGS()
    {
        if (zed.IsCameraReady && numberPoints != 0)
        {
            GaussianRFRenderPlugin.register_ZED_with_GS();
        }
    }
