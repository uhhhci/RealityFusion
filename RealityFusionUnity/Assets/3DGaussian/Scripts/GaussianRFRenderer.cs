...
public class GaussianRFRenderer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (leftHandle.ToInt32() != 0 && rightHandle.ToInt32() != 0 && rightHandleDepth.ToInt32() != 0 && leftHandleDepth.ToInt32() != 0 && !initialized)
        {
            ...
            GaussianRFRenderPlugin.update_proj_matrix_left(Matrix4fToArray(GSLeftCam.GetStereoProjectionMatrix(Camera.StereoscopicEye.Left)));
            GaussianRFRenderPlugin.update_proj_matrix_right(Matrix4fToArray(GSLeftCam.GetStereoProjectionMatrix(Camera.StereoscopicEye.Right)));

            Debug.Log("quest pro fov: " + GSLeftCam.fieldOfView);
            Debug.Log("quest pro aspect : " + GSLeftCam.aspect);

            // set no post proc tag
            GameObject rightController = GameObject.Find("Right_Right OpenVR Controller");
            GameObject leftController  = GameObject.Find("Left_Left OpenVR Controller");

            int targetLayer = LayerMask.NameToLayer("NoPostProc");
            if(rightController != null){rightController.layer = targetLayer;}
            if(leftController != null) { leftController.layer = targetLayer;}
    ...
    private void updateCameraPoses()
    {
        ...
        getXREyeTransform();
        GaussianRFRenderPlugin.transform_left_camera (new float[3] { leftEyePos.x, leftEyePos.y, leftEyePos.z });
        GaussianRFRenderPlugin.transform_right_camera(new float[3] { rightEyePos.x, rightEyePos.y, rightEyePos.z});

        
        Matrix4x4 leftCamViewMatrix  = Matrix4x4.TRS(leftEyePos, leftRot, Vector3.one).inverse;
        Matrix4x4 rightCamViewMatrix = Matrix4x4.TRS(rightEyePos, rightRot, Vector3.one).inverse;

        GaussianRFRenderPlugin.update_leftview_matrix(Matrix4fToArrayCamView(leftCamViewMatrix));
        GaussianRFRenderPlugin.update_rightview_matrix(Matrix4fToArrayCamView(rightCamViewMatrix));
    ...
    private void getXREyeTransform()
    {
        leftEyePos = InputDevices.GetDeviceAtXRNode(XRNode.LeftEye).TryGetFeatureValue(CommonUsages.leftEyePosition, out Vector3 leftEyePosValue) ? leftEyePosValue : Vector3.zero;
        rightEyePos = InputDevices.GetDeviceAtXRNode(XRNode.RightEye).TryGetFeatureValue(CommonUsages.rightEyePosition, out Vector3 rightEyePosValue) ? rightEyePosValue : Vector3.zero;
        leftRot  = InputDevices.GetDeviceAtXRNode(XRNode.LeftEye).TryGetFeatureValue(CommonUsages.leftEyeRotation, out Quaternion leftEyeRotValue) ? leftEyeRotValue : Quaternion.identity;
        rightRot = InputDevices.GetDeviceAtXRNode(XRNode.RightEye).TryGetFeatureValue(CommonUsages.rightEyeRotation, out Quaternion rightEyeRotValue) ? rightEyeRotValue : Quaternion.identity;

        leftEyePos  = (transform.position + leftEyePos) / renderScale;
        rightEyePos = (transform.position + rightEyePos) / renderScale;


    }

    private float getIPDFromXRPlugin()
    {
        getXREyeTransform();
