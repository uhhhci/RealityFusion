...
Shader "ZED/ZED PointCloud"
{
	Properties
	{
		...
		_MaxRange("Max Range", Range(1, 40)) = 40
	...
	SubShader
	{
		...
		Pass
		{
			...
			PS_INPUT vert (appdata_full v, uint vertex_id : SV_VertexID, uint instance_id : SV_InstanceID)
			{
				...
				o.color =  float4(tex2Dlod(_ColorTex, float4(uv, 0.0, 0.0)).bgr , 0.5f);
				o.size = _Size;
				return o;
			}
			...
			{
				// depth truncation: https://github.com/stereolabs/zed-unity/issues/122

				if (i.depth > _MaxRange) discard;
